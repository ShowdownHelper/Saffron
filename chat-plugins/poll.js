...
class Poll {
	constructor(room, questionData, options) {
		...
		this.startedUser = Prime.nameColor(questionData.username);
	...
	vote(user, option) {
		...
		//if (user.accountType === 'Untrusted') return user.sendTo(this.room, "You need to be autoconfirmed to vote in polls.");
